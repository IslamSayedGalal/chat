...
export default function ChatContainer(props) {
  ...
  const getAllMessages = async () => {
    ...
    console.log("user ::: ", user._id, props.currentChat._id)
  ...
  const handleSend = async (msg) => {
    ...
    updatedMessages.push({ from: user.role, text: msg });
    ...
    () => {
      if (props.socket.current) {
        ...
        props.socket.current.on("msg-recieve", (msg) => {
          setIncoming({ from: user.role, text: msg });
        });
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
            ...
            messages?.map((message) => {
              return (
                <div ref={scrollRef} key={uuidv4()}>
                  <div
                    className={`message ${
                      message.from === user.role ? "sended" : "recieved"
                    }`}
                  >
                    <p>{message.text}</p>
                  </div>
